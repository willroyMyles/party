...
class Store {
  @observable gotData: boolean = false;
  ...
  @action private getStreamToParties = async (
    limit?: number,
    datenum?: number,
  ) =>
    new Promise<boolean>(async (resolve) => {
      await FBS.events.linktorealTimeEvents(
        this.organizeStream,
        limit,
        datenum,
      );
  ...
  @action private getStreamToPastParties = async (
    limit?: number,
    datenum?: number,
  ) =>
    new Promise<boolean>(async (resolve) => {
      await FBS.events.linktoPastTimeEvents(
        this.organizePastEventsStream,
        limit,
        datenum,
      );
      resolve(true);
  ...
  @action private getLeaderboardPartyByType = (type: PartyType) =>
    new Promise<FeedItemModel[]>((resolve, reject) => {
      FBS.events
        .getLeaderboardPartyByType(type)
        .then((res) => {
  ...
  sortDataFromFireBase = (data: FirebaseFirestoreTypes.QuerySnapshot) => {
    ...
    return new Promise((resolve, reject) => {
      data.docs.forEach((doc, index) => {
        if (!this.data.has(doc.id) && !this.checkDate(doc.data().date))
  ...
  @action private getPastEvents = (reference: string) =>
    new Promise((resolve, reject) => {
      const amount = 5;
      FBS.events
        .getPastEvents(amount, reference)
        .then((res) => {
  ...
  @action private getEventsByRatings = () =>
    new Promise<FeedItemModel[]>((resolve, reject) => {
      const amount = 50;

      FBS.events
        .getEventsByRatings(amount)
        .then((res) => {
  ...
  @action getOnGoingParties = () => {
    return new Promise<FeedItemModel[]>((resolve) => {
      ...
      this.data.forEach((value, key) => {
        ...
        const finishTime = moment(new Date(value.date)).add(
          value.duration,
          'hours',
        );
  ...
  private getEventImageForReference = (reference: string, flyer: string) =>
    new Promise<string>(async (resolve, reject) => {
      if (this.data.has(reference)) {
        ...
        if (element.imageUrl != null || element.imageUrl != undefined)
          return resolve(element.imageUrl);
      }
  ...
  @action private picturesForEvent = (
    reference: string,
  ...
  @action private uploadPictureToEvent = (ref: string, imageUrl: string) => {
    return new Promise((resolve, reject) => {
      FBS.events
        .uploadPhotoToEvent(ref, imageUrl)
        .then((res) => {
          if (res) {
            resolve(res);
  ...
  @action private resetPassword = (email: string) =>
    FBS.auth.resetPassword(email);

  @action private getSpecificParties = (type: number, lastReference: string) =>
    new Promise((resolve, reject) => {
      FBS.events
        .getEventByType(type, lastReference)
        .then((res) => {
  ...
  @action private addRsvpEvent = (reference: string, add = true) =>
    new Promise((resolve, reject) => {
      FBS.events
        .uploadRsvpEvents(reference, add)
        .then((res) => {
  ...
  @action private checkUserLimitForPosting = () =>
    new Promise((resolve, reject) => {
      ...
      FBS.events
        ...
        .then((res) => {
          if (res) {
            ...
            Alert.alert(
              'oh my!',
              'We noticed users have been posting parties too often, as such we have set a limit on two posting per seven days. thank you for your understanding.',
            );
          }
        })
        .catch((err) => {
  ...
  @action private needsToBeLoggedInToProceed = () =>
  ...
  @action private moveData = () => {
    ...
    for (let index = 0; index < arr.length; index++) {
      ...
      if (goAhead)
        FBS.events.moveEventsAround(element.reference, element).then((res) => {
          if (res) {
  ...
  @action private increaseAttendance = (key: string) =>
    new Promise<boolean>((resolve, reject) => {
      FBS.events
        .increaseAttendance(key)
        .then((res) => {
          resolve(res);
        })
        .catch((err) => {
