...
export const HomeNavigator = observer(() => {
  ...
  useEffect(() => {
    ...
    eventEmitter.addListener('noti', navigateFromNotification);
    return () => {
      Linking.removeEventListener('url', listenerForLinking);
      eventEmitter.removeListener('noti', navigateFromNotification);
    };
  }, []);

  const navigateFromNotification = () =>
    nav.navigate('view event', {reference: FireStore.lastKnownUrl});
  ...
  const listenerForLinking = ({url}: {url: string}) => {
    ...
    if (FireStore.data.has(path)) {
      ...
      TToast.error(
        'Oh my',
        'this party has either expired or removed by the owner',
      );
    }
  };

  const dissectUrl = (urlToDissect: string) => {
    if (urlToDissect.includes('http'))
      return urlToDissect.replace('http://myparty.com/', '');
