...
const CategoryView = () => {
  ...
  const setLast = () =>
    new Promise<FeedItemModel[]>((resolve) => {
      const d = getValues(partyType);
  ...
  const getValues = (type1: number) => {
    const d: FeedItemModel[] = [...FireStore.data.values()].filter(
      (value, index) => {
  ...
  const loadData = () => {
    ...
    setLoading(true);
    FireStore.retrieve
      .specificParties(partyType, lastDocument || '')
      .then((res) => {
        setLoading(false);
        setLast().then((res) => {
          updateParties(res);
    ...
    return (
      <SafeAreaView style={{flex: 1, backgroundColor: Colors.background}}>
        <View bg-background flex center>
          <Text marginV-12 indicator>
            {GetPartytypeString(partyType)}
          </Text>
          <FlatList
            data={[...data]}
          ...
          {loading && (
            <View center margin-10 style={{minHeight: 40}}>
